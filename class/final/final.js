...
function changePhone3(){
    ...
    if(phone1.length === 3 && phone2.length === 4 && phone3.length === 4){
        document.getElementById("codeButton").style = "background-color: #FFFFFF; color: #0068FF; cursor: pointer;"
        document.getElementById("codeButton").removeAttribute("disabled")
    }
...
function getToken(){
    ...
    document.getElementById("codeButton").style = "background-color: #FFFFFF; cursor: default;"
    document.getElementById("codeButton").setAttribute("disabled", "true")
    document.getElementById("confirmButton").style="background-color: #0068FF; color: #FFFFFF; cursor: pointer;"
...
function getTokenTimer(){
    ...
    interval = setInterval(() => {
        if(timer >= 0){
            ...
            document.getElementById("confirmNum").innerText = minutes + ":" + String(seconds).padStart(2, "0")
            timer -= 1
        } else {
            document.getElementById("codeNumber").innerText = "000000"
            document.getElementById("codeButton").style = ""
            document.getElementById("codeButton").setAttribute("disabled", "true")

            document.getElementById("confirmNum").innerText = "3:00"
...
function getTokenTimerConfirm(){
    clearInterval(interval)
    document.getElementById("confirmButton").style = "background-color: #FFFFFF; cursor: default;"
    document.getElementById("confirmButton").setAttribute("disabled", "true")
    document.getElementById("confirmButton").innerText = "인증 완료"
    alert("인증이 완료되었습니다.")

    document.getElementById("submit").style = "background-color: #FFFFFF; color: #0068FF; border: 1px solid #0068FF ;cursor: pointer;"
